...
export default function Navbar({ children }) {
    ...
    useEffect(() => {
        ...
        if (u) {
            ...
            // Tërhiq workspaces dinamike për menunë anësore
            api.get(`/workspaces/${parsed.id}`)
                .then(res => setWorkspaces(res.data))
                .catch(err => console.error(err));
        } else {
            navigate('/login');
        }
    }, [navigate, location.pathname]); // Rifresko kur ndryshon faqja në rast se shtuam workspace të ri
        ...
        <div className="flex h-screen bg-[#f8fafc] overflow-hidden font-sans">
            ...
            <div className="w-64 bg-white border-r border-[#e5e7eb] flex flex-col justify-between shadow-sm z-10 shrink-0">
                    ...
                    <div className="px-4 py-6">
                        ...
                        <div className="mb-8">
                            <div className="flex items-center justify-between px-3 text-xs font-bold uppercase tracking-wider text-slate-400 mb-2">
                                ...
                                <Plus className="w-4 h-4 cursor-pointer hover:text-slate-600" title="Krijo të ri" onClick={() => navigate('/workspaces')} />
                            </div>
                            <div className="space-y-1">
                                {workspaces.length === 0 ? (
                                    <div className="px-3 py-2 text-xs text-slate-400 italic">No workspaces yet.</div>
                                ) : (
                                    workspaces.map(ws => (
                                        <Link
                                            key={ws.id}
                        ...
                        <div className="mb-8">
                            ...
                            <div className="space-y-1">
                                <Link
                                    to="/users"
                                    className={`flex items-center gap-3 px-3 py-2.5 rounded-xl font-medium transition-colors ${
                                        location.pathname === '/users' ? 'bg-[#0f172a] text-white shadow-md' : 'text-slate-600 hover:bg-slate-50'
                                    }`}
                                >
                                    <Users className={`w-4 h-4 ${location.pathname === '/users' ? 'text-white' : 'text-slate-400'}`} />
                                    All Users (IDs)
                                </Link>
                            </div>
                        </div>
                ...
                <div className="px-4 py-4 border-t border-[#e5e7eb] flex justify-between items-center group cursor-pointer hover:bg-slate-50 rounded-xl transition-colors m-2">
                    <div className="flex items-center gap-3 overflow-hidden flex-1" onClick={() => navigate('/profile')}>
                    ...
                    {/* Logout Button inside the profile area */}
                    <button 
                        onClick={(e) => {
                            e.stopPropagation();
                            localStorage.clear();
            ...
            <div className="flex-1 flex flex-col overflow-hidden relative">
                ...
                <header className="h-20 border-b border-[#e5e7eb] bg-white flex items-center justify-between px-8 z-10 sticky top-0 shrink-0">
                    <div className="flex-1 max-w-xl">
                        <div className="relative border border-slate-200 rounded-2xl overflow-hidden focus-within:ring-2 focus-within:ring-slate-200">
                            <input
                                type="text"
                            ...
                            />
                            <svg className="absolute left-4 top-2.5 w-5 h-5 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div className="flex items-center gap-4">
                        <button className="flex items-center gap-2 bg-[#0f172a] text-white px-5 py-2.5 rounded-2xl text-sm font-semibold hover:bg-slate-800 transition-colors shadow-sm" onClick={() => navigate('/tasks')}>
